...
def get_not_recognisable_users():
    not_recognisable_users = db.session.query(User).join(Relation).filter(Relation.mirror_id == me.id, Relation.recognisable == False).all()
    return not_recognisable_users
